...
//radius of the gear part that we will be pushing
double gearRadius = 0.04;

//radius of the tool flange
double flangeRadius = 0.03;

//goal location to push the gear to
double goalXPos = 0.4;
double goalYPos = 0.0;
...
int main(int argc, char** argv) {
    ...
    //first, line up with the gear on the x axis
    //use x and y coordinates of the gear part, offset by gear and flange radius
    flange_origin << g_perceived_object_pose.pose.position.x - gearRadius - flangeRadius, g_perceived_object_pose.pose.position.y, 0.5; //specify coordinates for the desired flange position (origin) with respect to the robot's base frame
    ...
    //go to pose to touch side of gear part; same x and y, lower z.
    flange_origin <<g_perceived_object_pose.pose.position.x - gearRadius - flangeRadius, g_perceived_object_pose.pose.position.y, 0.0; 
    ...
    //push in the x, same y and z, goal x is goal - gear radius - flange radius
    flange_origin << goalXPos - gearRadius - flangeRadius, g_perceived_object_pose.pose.position.y, 0.0;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    ...
    ros::Duration(2.0).sleep(); //wait for the motion
    ...
    //go up a bit so we don't hit the gear while pushing it in the y
    flange_origin << goalXPos - gearRadius - flangeRadius, g_perceived_object_pose.pose.position.y, 0.2;
    goal_flange_affine.translation() = flange_origin;
    ROS_INFO_STREAM("move to flange origin= " << goal_flange_affine.translation().transpose() << endl);
    g_q_vec_arm_Xd = optimal_path.back(); //start from the joint-space pose that ended the prior plan
    //convert move to an optimal joint-space path:
    ...
    //move to the position needed to push the gear in the y, above the gear
    //figure out if the gear needs to be pushed in the -y or +y
    if(g_perceived_object_pose.pose.position.y > 0){
        //gear needs to be pushed in the -y, so flange needs to have greater y
        flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + gearRadius + flangeRadius, 0.2;
    }
    else{
        //gear needs to be pushed in the +y, so flange needs to have smaller y
        flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - gearRadius - flangeRadius, 0.2;
    }
    ...
    //move straight down to reach the z needed to push the gear
    //figure out if the gear needs to be pushed in the -y or +y
    if(g_perceived_object_pose.pose.position.y > 0){
        //gear needs to be pushed in the -y, so flange needs to have greater y
        flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y + gearRadius + flangeRadius, 0.0;
    }
    else{
        //gear needs to be pushed in the +y, so flange needs to have smaller y
        flange_origin << g_perceived_object_pose.pose.position.x, g_perceived_object_pose.pose.position.y - gearRadius - flangeRadius, 0.0;
    ...
    //finally, push the gear towards the goal position
    //figure out if the gear needs to be pushed in the -y or +y
    if(g_perceived_object_pose.pose.position.y > 0){
        //gear needs to be pushed in the -y, so flange needs to have greater y
        flange_origin << g_perceived_object_pose.pose.position.x, goalYPos + gearRadius + flangeRadius, 0.0;
    }
    else{
        //gear needs to be pushed in the +y, so flange needs to have smaller y
        flange_origin << g_perceived_object_pose.pose.position.x, goalYPos - gearRadius - flangeRadius, 0.0;
